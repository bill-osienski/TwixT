...
- **12.2 Immutable frozen-check coefficient + split fingerprint (Task 7).** The `frozen_check` candidate must equal the `smallest_safe_r` selected on the tuning split. Because tuning deliberately includes selected-A while frozen forbids it (§12.3), a full-fingerprint equality is wrong — split the fingerprint:
  - **selection-context fingerprint (shared, hard-match):** source-file hashes (§12.5) / checkpoint hash / dev-manifest hash / base effective MCTS config / seeds / the frozen grid. Frozen must match tuning's selection-context EXACTLY.
  - **stage-specific run fingerprint (recorded, not cross-matched):** tuning records selected-A PRESENT; frozen records selected-A ABSENT; plus the §12.5 runtime provenance.
  `run_candidates_stage(--mode frozen_check)` REQUIRES a tuning selection artifact with a non-null `smallest_safe_r`, validates the shared selection-context matches, and REJECTS any `--frozen-r` that differs from that selected coefficient. ("One nonzero r" alone permits an arbitrary value.)
...
- **12.4 Complete persisted candidate artifacts + VERIFIED recompute (Task 7).** Persist, not just pass/fail reasons: per-position `candidate-vs-absolute_off` AND `candidate-vs-r0` rows (joinable by `canonical_sha1`); selected-A case rows; full NUMERIC §6.2/§6.3 gate summaries (every computed metric, both references); and the selected-coefficient record. **Join-vs-recompute = recompute, but VERIFIED (not asserted):** the candidate stage recomputes `absolute_off` and `r0` (necessary — `controls_cases.csv` omits `top_move`/`top_move_prior` for the low-prior-flip gate). It must then, per `(canonical_sha1, config)`, compare EVERY field present in `controls_cases.csv` against the freshly-recomputed row and **ABORT on any mismatch**. Record `controls_provenance: "recomputed_and_verified"` plus the compared-row count and a hash of the compared rows.

- **12.5 Strengthened fingerprints (Task 7 + builder).** A git commit alone does not detect uncommitted edits, and a replay PATH does not fingerprint replay CONTENTS. Every fingerprint must include: **source-file hashes** of the effective result-determining modules — the diagnostic/builder (`diagnose_fpu_policy_mass.py`, `build_fpu_dev_corpus.py`), the search core (`mcts.py`), AND the state-RECONSTRUCTION dependencies (`goal_line_trigger_probe_cases.py`, `game/twixt_state.py`), which are equally result-determining because they rebuild the exact position each search runs on; the builder additionally hashes `fpu_state_hash.py` (its canonical-sha1 source of truth) — + a clean-worktree flag + git commit; source-index hash + a deterministic replay-DATA hash (contents, not paths, with each file's bytes length-delimited so a byte-repartition across replays cannot collide); checkpoint hash; the FULL effective MCTS config (`dataclasses.asdict`, not a subset); an explicit `add_noise=False` record; and **runtime provenance** — Python version, MLX/relevant-package version, platform/device identity (recorded even if not a hard equality gate; belongs to the stage-specific run fingerprint, not the shared selection-context).

  **12.5.1 VERIFIED-recompute artifacts (Task 7, #4).** The candidate stage additionally: (a) recomputes absolute_off/r0 and VERIFIES every persisted `controls_cases.csv` field against the fresh recompute per `(canonical_sha1, config)`, aborting on any mismatch, and records `controls_provenance: "recomputed_and_verified"` + `controls_verified_row_count` + `controls_verified_rows_sha1`; (b) exposes every computed §6.2 gate number via `SafetyVerdict.metrics` (gate LOGIC unchanged) and persists `metrics_vs_absolute_off`/`metrics_vs_r0`/`selected_a_metrics` per candidate plus `smallest_safe_reduction`; (c) persists the joinable per-position evidence — `candidate_dev_rows.csv` (candidate-vs-absolute_off AND candidate-vs-r0, keyed by `canonical_sha1`) and `selected_a_cases.csv` (tuning only). The persisted controls CSV round-trips floats exactly (Python-3 shortest-repr, `None`→`""`), so verification is bit-exact.
