...
- **The only new code is a read-only manifest builder** that reuses v6's `continuation_extraction.py` to extract the searched children at A roots and emit them as correction rows. This mirrors how v7 was a manifest-only branch on the v9 surface.

| File | Change |
|---|---|
| `scripts/GPU/alphazero/build_v15_a_continuation_correction_manifest.py` (new) | read-only builder: per A pre-drop root, run BASE 400-sim MCTS, extract top-visited child/PV states via `continuation_extraction`, emit `hard_value` correction rows tagged `black_predrop_continuation_correction`; also emits the Phase-0 concentration diagnostic CSV |
| manifest (new, under `logs/eval/`) | v12b manifest + the appended A-continuation correction rows |
| schedule (operator arg) | add `black_predrop_continuation_correction`; keep B/C/D guardrails unchanged |
| `smoke_v15_*.py` (new) | pool-sampling smoke: the new tag draws, is value-only, mask-aligned, no NaN |
...
Output: `logs/eval/v15prep_a_continuation_concentration.csv` (per-root, per-child: `visit_share`, `child_contribution_share`, depth, raw values BASE/v14b, root MCTS value) + a one-paragraph classification read in the ledger. **No training until this CSV is reviewed.**
...
- **Board baking (why no new training/loader code):** the builder reconstructs each child position (root board + the extracted move sequence) and stores it directly in the row's `board_tensor`, so each continuation row reaches training as a **plain `hard_value` row at a child board** — no `extra_moves_json` continuation-reconstruction path is exercised at train time, so `build_calibration_position`, the loader, and the loss are byte-identical to the root A-correction path. The reconstruction lives entirely in the (read-only) builder, which already holds the MCTS tree.
