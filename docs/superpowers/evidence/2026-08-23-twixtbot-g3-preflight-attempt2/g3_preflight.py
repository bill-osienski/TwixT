...
def main():
    ...
    def reference_agent_binds_everything():
        """The reference must bind the SCHEDULED seed, checkpoint and settings."""
        cfg = RF.eval_config()
        ...
        task = S.enumerate_tasks()[0]
        ev = RF.load_reference_evaluator(task["reference"], repo)   # verifies sha1
        ag = RF.build_reference_agent(task=task, evaluator=ev, colour="black")
        assert ag.seed == task["seed"] == 202611000, f"seed {ag.seed}"
        import random
        assert ag.mcts.rng.getstate() == random.Random(task["seed"] ^ 0x5A5A5A).getstate(), \
            "search RNG is not the eval_runner black stream for this seed"
        assert ag.readout_rng.getstate() == random.Random(task["seed"] ^ 0x3C3C3C).getstate(), \
            "readout RNG is not the eval_runner black stream for this seed"
        assert ag.mcts.config.n_simulations == 400
        # a wrong-colour call must refuse rather than play
        ...
        return (f"seed {ag.seed} bound; both RNG streams match eval_runner's masks; "
                f"400 sims / batch 14 / flush 48 / opening_temperature; out-of-turn refused")

    def reference_one_move():
        """One real reference move at 400 sims, validated in our engine."""
        task = S.enumerate_tasks()[0]
        ev = RF.load_reference_evaluator(task["reference"], repo)
        ag = RF.build_reference_agent(task=task, evaluator=ev, colour="black")
        our = TwixtState(max_plies_limit=S.PLY_CAP)
    ...
    def no_g3_game_played():
        ...
        return (f"no scheduled task was run and no reserved seed consumed; the {hi-lo} seeds in "
                f"[{lo}, {hi}) are untouched. play_game was invoked TWICE, both on the SAME "
                f"synthetic task (seed -1) with fake engines, to observe abort behaviour; "
                f"single real moves were played by each engine, but no game.")
