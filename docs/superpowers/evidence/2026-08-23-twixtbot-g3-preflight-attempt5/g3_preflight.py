...
def main():
    ...
    def gate_is_durable_and_exit_bound():
        """The verdict must be persisted AND expressed as an exit status."""
        import tempfile
        from scripts.GPU.alphazero import twixtbot_g3_runner as RUN
        assert (RUN.EXIT_GATE_PASSED, RUN.EXIT_GATE_FAILED,
                RUN.EXIT_RUN_ABORTED, RUN.EXIT_PRECONDITION) == (0, 1, 2, 3)
        assert RUN.gate_verdict({"completed": False}) == "ABORTED"
        assert RUN.gate_verdict({"completed": True, "summary": {"selected_trials": None}}) == "FAILED"
        assert RUN.gate_verdict({"completed": True, "summary": {"selected_trials": 100}}) == "PASSED"
        ...
        # the terminal record is appended and fsynced
        ...
        assert rec["summary"]["selected_trials"] is None, rec
        # the CLI refuses a bad precondition with exit 3 rather than raising
        code = RUN.main(["--clone-root", "/nonexistent", "--repo-root", repo,
                         "--results-path", os.path.join(tempfile.mkdtemp(), "x.jsonl")])
        assert code == RUN.EXIT_PRECONDITION, f"CLI returned {code}"
        return ("verdict persisted as a terminal record; PASSED/FAILED/ABORTED map to exit "
                "0/1/2; a refused precondition exits 3")
