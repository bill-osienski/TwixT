...
def main():
    ...
    def make_ply_check(label, moves):
        def run():
            ...
            d = A.state_divergences(our, tb, twixt, ply_cap=None)
    ...
    def end_to_end():
        ...
        assert not A.state_divergences(our, tb, twixt, ply_cap=None), "pair diverged before the query"
        ...
        d = A.state_divergences(our2, tb, twixt, ply_cap=None)
    ...
    def checker_binds():
        ...
        assert not A.state_divergences(our, tb, twixt, ply_cap=None), "premise: pair agrees"
    ...
    def ply_cap_is_required():
        """Omitting ply_cap must RAISE, not default.

        A default of None is a guard the caller can switch off by omission. This
        asserts the signature enforces the decision rather than the convention
        being remembered, and that both explicit choices still work.
        """
        our, tb = A.build_pair([(12, 12), (11, 10)], twixt, Point, TwixtState)
        try:
            A.state_divergences(our, tb, twixt)          # no ply_cap
        except TypeError as e:
            assert "ply_cap" in str(e), f"TypeError does not name ply_cap: {e}"
        else:
            raise AssertionError("omitting ply_cap did NOT raise; the default is back")
        # positional passing must also be refused (keyword-only)
        try:
            A.state_divergences(our, tb, twixt, 280)
        except TypeError:
            pass
        else:
            raise AssertionError("ply_cap was accepted positionally; it is not keyword-only")
        # both explicit choices still work
        assert A.state_divergences(our, tb, twixt, ply_cap=None) == []
        assert A.state_divergences(our, tb, twixt, ply_cap=280) == []
        import inspect
        sig = inspect.signature(A.state_divergences)
        prm = sig.parameters["ply_cap"]
        assert prm.kind is inspect.Parameter.KEYWORD_ONLY, f"ply_cap kind is {prm.kind}"
        assert prm.default is inspect.Parameter.empty, f"ply_cap has default {prm.default!r}"
        return "omission and positional both raise TypeError; keyword-only, no default"

    print("\nsignature binding:")
    ok &= check("ply_cap is required and keyword-only", ply_cap_is_required)
