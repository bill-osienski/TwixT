...
2. **`run_atlas preflight --checkpoint <net> --pilot-dir <pilot_block>`** — measures the
   worktree, HEAD and checkpoint. Exit `0` continue · `2` usage · `3` PROVENANCE_FAILURE.
3. **Generate `[0, 24)`.** One block directory, which fails closed if it already exists.
4. **`build_atlas_corpus pilot-gate`.** A `PHASE_GEOMETRY_NO_GO` means **stop** — not a
   smaller pilot, not replacement games, not reassignment.
5. **`run_atlas run-pilot`**, launched through the detached shell wrapper the runbook
...
6. **Read both sidecars, in a later call.** Do not try to wait on the PID — after
