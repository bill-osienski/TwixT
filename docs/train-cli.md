...
- `probe_summary`: tier-keyed structure produced alongside the legacy field for forward compatibility:
  - `probe_summary.forced` — same payload as `forced_probe_summary` (mirror during the dual-emit window).
  - `probe_summary.strong_advantage` — placeholder slot for the strong-advantage tier; populated when an inline strong-advantage eval lands in a follow-up plan. `null` today.

  Downstream readers (analyzer, dashboards) should prefer `probe_summary.<tier>` over `forced_probe_summary`. The legacy field will be removed after one release cycle of dual emission.

### Probe tiers (forced and strong_advantage)

The trainer's per-iter probe eval is **scoring** against a pre-built probe suite — it is NOT computing labels at training time. Probe suites are produced offline by `scripts/build_probe_suite.py` (see [`docs/probe-suite-generation.md`](probe-suite-generation.md)) and committed to `tests/probes/`. The trainer reads them and reports how well each candidate checkpoint's value head agrees with the suite's ground-truth labels.

Two tiers are supported today:

- **Forced** (`tests/probes/twixt_probes.json`) — positions 1–2 plies from a forced terminal win. Used as a basic sanity floor (gate target ≥95% sign-correct). Refreshed automatically when training generates new game data; the bootstrap suite generator (`scripts/build_bootstrap_probe_suite.py`, now a shim that forwards to `build_probe_suite.py --tier forced`) regenerates it.
- **Strong-advantage** (`tests/probes/strong_advantage_probes.json`) — positions where one side has built a strong but not-yet-forced chain advantage (3–8 plies from terminal). Used to detect value-head bias on dominant-but-not-trivial positions (gate target ≥80% sign-correct, but **not gated yet** — first measure, then choose a threshold). Generated by `scripts/build_probe_suite.py --tier strong_advantage` with deep-MCTS labeling; see the dedicated doc for the full operator workflow.

**When the strong-advantage tier rerun is warranted:**

1. **A new "champion" checkpoint emerges with the value head clearly out-of-distribution from the current label authority.** The current suite was labeled by `model_iter_0059`. If a checkpoint many iterations later (say, iter 200+) has materially shifted policy, its MCTS-derived labels would be more trustworthy on positions the older model couldn't yet evaluate well — refresh against the newer checkpoint.
2. **Source-replay corpus gets expanded with materially different distributions.** If self-play parameters change (curriculum bumps, dirichlet alpha shifts, opening exploration changes), regenerate to mine candidates from the new distribution.
3. **Persistent low scores on the existing suite even after extended training.** If `probe_summary.strong_advantage.sign_correct_pct` plateaus below 60% for 20+ iterations and no clear quality regression is visible elsewhere, suspect the suite's labels — the labeling MCTS may have inherited the same value-head bias as the under-trained model. Refresh with a stronger checkpoint.
4. **Major architecture changes.** Channel-count bumps, hidden-size changes, etc., warrant a fresh probe suite (the `phase2_label.label_checkpoint_sha256` in the meta block tracks the labeling-network identity).

In normal operation, **the suite does not need frequent regeneration** — once-every-50-to-100-iterations is plenty. The whole point is to have a stable yardstick across many training iterations, so that improvements (or regressions) show up clearly.

**See also:** [`docs/analysis-metrics-guide.md`](analysis-metrics-guide.md) for how to interpret the output files (game JSON, sidecar stats, analyzer summary/CSVs), and [`docs/probe-suite-generation.md`](probe-suite-generation.md) for the operator workflow on regenerating either tier.
