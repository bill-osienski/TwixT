...
50%. The frozen disposition decided all three outcomes in advance: **bar not met ⇒ close
...
| as red | 97–97 + 6 caps, 0.5000, CI95 `[0.4318, 0.5682]` — **neutral**, rule does not fire |
| as black | 73–127 + 0 caps, 0.3650, CI95 `[0.2983, 0.4317]` — **harm fires**, upper bound below `0.50` |
| colour safety | **black-specific.** The aggregate loss and the black harm **each reject independently** |
...
Unlike the cold run, where both colours rejected, harm here is **confined to black**; red came
out exactly even. The cap rate, absence of board-full endings and the decisive red win rate
give no sign that the loss is a termination or colour-allocation artifact.
...
one cold, one warm — both land below it, and that removing the cold-start shock recovered much
