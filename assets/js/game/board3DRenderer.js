export default class Board3DRenderer {
    ...
    this.renderer.domElement.addEventListener('click', (event) => {
      ...
      if (intersects.length > 0) {
        ...
        if (this.game.placePeg(row, col)) {
          if (
            this.gameController &&
            typeof this.gameController.recordMove === 'function'
          ) {
          ...
          if (this.game.gameOver && this.game.winner) {
            ...
            const aiStatus =
              this.game.isAIGame && this.game.winner === this.game.aiPlayer
                ? ' (AI)'
                : '';
            setTimeout(() => {
  ...
  isValidPosition(row, col) {
    if (
      (row === 0 || row === this.game.boardSize - 1) &&
  ...
  drawBridge(from, to, player) {
    ...
    const bridgeGeometry = new THREE.CylinderGeometry(
      ...
      0.08,
      distance - 0.4
    ); // Slightly shorter to account for end caps
