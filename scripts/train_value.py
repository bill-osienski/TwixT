...
FEATURE_KEYS = [
    'friendlyConnections', 'opponentConnections', 'friendlyDistance', 'opponentDistance',
    'goalDistance', 'centerBias', 'isolatedBonus', 'chainProximity', 'frontierProximity',
    'frontierCapture', 'connectorProximity', 'connectorCapture', 'trailingPenalty',
    'threatReduction', 'noThreatReduction', 'spanGain', 'blackSpanComplete',
    'redSpanComplete', 'opponentSpanReduction', 'noSpanReductionPenalty',
    'blackSpanUpgradePenalty', 'redSpanUpgradePenalty', 'topBias', 'aboveMinRowBonus',
    'belowMinRowPenalty', 'bottomBias', 'belowMaxRowBonus', 'aboveMaxRowPenalty'
]
...
def flatten_features(move: Dict[str, Any], summary: Dict[str, Any]) -> Tuple[List[float], int]:
    """
...
def load_dataset(path: str, gamma: float = 1.0) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
...
def weighted_logloss(y_true: np.ndarray, p: np.ndarray, w: np.ndarray) -> float:
    ...
    return float((-(w * (y_true * np.log(p) + (1 - y_true) * np.log(1 - p)))).sum() / w.sum())
...
def main():
    parser = argparse.ArgumentParser(description='Train a simple value model from self-play traces.')
    parser.add_argument('--input', required=True, help='Path to self-play JSON trace (from scripts/selfPlay.js).')
    parser.add_argument('--output', default='value-model.json', help='Where to save the trained model weights.')
    ...
    parser.add_argument('--l2', type=float, default=0.0,
                        help='L2 regularization strength (e.g., 1e-4).')
    ...
    if X.size == 0:
        raise SystemExit("No training samples found in the input file (maybe all games were draws?).")
