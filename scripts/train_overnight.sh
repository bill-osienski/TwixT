...
echo ""
echo "3. Pin it: create models/<content-address>/ holding the graph, its"
echo "   model.onnx.data sidecar and a manifest.json recording both SHA-256s."
echo "   The server loads only from a manifest; there is no export on startup."
echo ""
echo "4. Start the server against a staged manifest:"
echo "   MODEL_MANIFEST=models/<content-address>/manifest.json npm run server"
