...
def run_replay_warmup(
    ...
    **selfplay_kwargs,
    ...
    """Fill `buffer` with `n_games` self-play games before any optimizer step.

    Opt-in bootstrap for a continuation run. The games are generated by whatever
    weights are already loaded, so under `--load-weights` they are parent games.
    Nothing is trained here and nothing is persisted: the positions live only in
    ...
    if n_workers < 2:
        raise ValueError(
            f"replay warmup ({n_games} games) requires n_workers >= 2: warmup "
...
def train(
    ...
    if replay_warmup_games > 0:
        print(f"  Replay warmup: {replay_warmup_games} games before the first training step")
    ...
    for iteration in range(start_iteration, n_iterations):
        ...
        # 0. Opt-in parent-replay bootstrap. Runs once, before the first optimizer
        # step, with the weights as loaded — so the buffer starts warm instead of
        # empty. Consumes master_rng in place, so this iteration's own self-play
        # continues from the advanced stream. Default 0 leaves everything below
        # untouched.
        # ponytail: warmup games record into the curriculum window like ordinary
        # games. With several --curriculum-sizes a large warmup could promote the
        # size before iteration 0; this experiment pins one size. Pass a scratch
        # CurriculumManager here if that ever matters.
        if iteration == start_iteration and replay_warmup_games > 0:
            network.eval()
            print(f"\nReplay warmup: generating {replay_warmup_games} games with the "
                  f"loaded network ({n_workers} workers, no training)...")
            _warmup_start = time.perf_counter()
            _warmup_positions = run_replay_warmup(
                replay_warmup_games,
                n_workers=n_workers,
                buffer=buffer,
                master_rng=master_rng,
            ...
            print(f"  Replay warmup: {_warmup_positions} positions added, "
                  f"buffer={len(buffer)}, {time.perf_counter() - _warmup_start:.1f}s")
