...
def assert_self_distillation(network, manifest_path: str, tol: float = 1e-4) -> dict:
    ...
    # The network uses BatchNorm: a TRAIN-mode forward normalizes by per-BATCH
    # statistics, so a position's value/policy would depend on the batch it is in
    # — and the teacher cache is built per-position. Run the whole check in EVAL
    # mode (running stats) so the forward is batch-independent and reproduces the
    # eval-built cache exactly. (Eval mode also does not mutate running stats, so
    # the ignored main-batch forward cannot perturb the calibration forward.)
    prev_training = network.training
    network.eval()
    try:
        # The main-batch losses are IGNORED here — we only read the calibration
        # outputs. Reusing `records` as the main batch avoids constructing unrelated
        # dummy positions.
